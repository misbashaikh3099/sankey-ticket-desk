...
export const reportApi = {
  getStats: async () => {
    const res = await api.get('/reports/stats');           // ← fixed
    return res.data;
  },

  getPriorityStats: async () => {
    const res = await api.get('/reports/priority');        // ✅ already correct
    return res.data;
  },

  getVendorPerformance: async () => {
    const res = await api.get('/reports/vendor-performance'); // ← fixed
    return res.data;
  },

  getSlaReport: async () => {
    const res = await api.get('/reports/sla');             // ✅ already correct
    return res.data;
  },
};
